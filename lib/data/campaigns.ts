...
const RAW_CAMPAIGN = {
  ...
  /**
   * Texto do projeto, um parágrafo por item: o primeiro abre a página abaixo
   * do CTA (`campaign-progress`), o segundo acompanha a capa na vitrine e o
   * resto vira o texto corrido depois do parallax (ver `campaign-about`).
   *
   * Nada aqui repete o que a própria página já mostra em bloco próprio
   * (2026-09): a bio de Alcatena e Mazzitelli sai de "Sobre o Autor"
   * (`author.bio`, em `books.ts`), o convite ao leitor sai de
   * `recommendedIntro`/`recommendedFor`, e a sinopse já é `description`. Os
   * parágrafos que repetiam esses três blocos foram removidos — ao trazer
